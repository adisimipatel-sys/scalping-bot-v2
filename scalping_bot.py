...
# Public Binance data endpoint (no geo-restriction)
BINANCE_DATA_URL = "https://data.binance.com"

def get_candles(symbol: str, interval="1m", limit=150) -> list:
    """Fetch 1-minute candles from data.binance.com (public, no geo-restriction)"""
    try:
        ...
        if not isinstance(data, list) or len(data) == 0:
            logger.error(f"Candles error {symbol}: {data}")
            return []
        return [{
            "time":   int(c[0]) / 1000,
            "open":   float(c[1]),
...
def get_price(symbol: str) -> Optional[float]:
    """Get current price from data.binance.com (public)"""
    try:
        url = f"{BINANCE_DATA_URL}/api/v3/ticker/price"
...
def get_order_book(symbol: str) -> dict:
    """Get order book from data.binance.com"""
    try:
        url = f"{BINANCE_DATA_URL}/api/v3/depth"
        r = requests.get(url, params={"symbol": symbol, "limit": 20}, timeout=10)
        data = r.json()
        if "bids" not in data:
